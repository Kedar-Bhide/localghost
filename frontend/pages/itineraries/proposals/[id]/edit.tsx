...
export default function EditProposal() {
  ...
  useEffect(() => {
    if (proposal) {
      setFormData({
        ...
        daily_itinerary: proposal.daily_itinerary,
        price_breakdown: proposal.price_breakdown || [],
        included_services: proposal.included_services,
  ...
  const fetchProposalDetails = async () => {
    try {
      ...
      if (data.local_id !== user?.id) {
        throw new Error('You can only edit your own proposals');
      }

      // Check if proposal can be edited
      if (!['draft', 'withdrawn'].includes(data.status)) {
        throw new Error('This proposal cannot be edited in its current status');
      }
  ...
  const handleSubmit = async (e: React.FormEvent, saveAsDraft = false) => {
    ...
    try {
      ...
      // If not saving as draft and current status is draft, also submit
      if (!saveAsDraft && proposal?.status === 'draft') {
        await fetch(`/api/v1/itineraries/proposals/${id}/status`, {
          method: 'PATCH',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
      ...
      router.push(`/itineraries/proposals/${id}`);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'An error occurred');
    } finally {
      setSubmitting(false);
  ...
  return (
    <>
      <Head>
        <title>Edit Proposal - LocalGhost</title>
      </Head>
      ...
      <div className="min-h-screen bg-gray-50">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="mb-8">
            ...
            <h1 className="text-3xl font-bold text-gray-900">Edit Itinerary Proposal</h1>
            <p className="mt-2 text-lg text-gray-600">
              Update your proposal details and itinerary
            </p>
          </div>
